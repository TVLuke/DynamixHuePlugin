...
public class HueVirtualLightGroup implements HueLight {
	...
	public HueVirtualLightGroup(HueBridge bridge, Integer id, String name, HueLight... lights) {
		...
		this.id = id;
		this.name = name;
		this.lights = new LinkedHashSet<HueLight>();
	...
	public Collection<? extends HueLight> getLights(Integer id) {
		final ArrayList<HueLight> result = new ArrayList<HueLight>();
		for(HueLight light : lights) {
			if(light.getId().equals(id)) {
				result.add(light);
			}
	...
	@Override
	public void setState(Boolean on, Integer brightness, Integer hue, Integer saturation) {
		if(brightness<0 || brightness>255) {
			throw new IllegalArgumentException("Brightness must be between 0-255");
		}
		if(hue<0 || hue>65535) {
	...
	public void stateChangeTransaction(Integer transitionTime, Runnable changes) {
		final Set<HueLight> lightsForTransaction = new LinkedHashSet<HueLight>();
		...
		final ArrayList<HueLight> reversedList = new ArrayList<HueLight>(lightsForTransaction);
