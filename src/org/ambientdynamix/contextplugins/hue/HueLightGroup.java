...
public class HueLightGroup implements HueLight {
	...
	HueLightGroup(HueBridge bridge, Integer id, Map<Integer, HueLightBulb> lights) 
	{
		...
		{
			this.lights = new TreeMap<Integer, HueLightBulb>();
		}
	}
	...
	public void stateChangeTransaction(Integer transitionTime, Runnable changes) {
		...
		try {
			...
			System.err.println("throw: "+t.getMessage());
			//throw t;
		} finally {
	...
	private List<JSONObject> stateChange(Map<String,Object> param_value) {
		...
		if(stateTransaction==null) {
			...
			return bridge.checkedSuccessRequest(PUT, "/groups/" + getId() + "/action", json);
		} else {
			for (String s : param_value.keySet()) {
